...
const spawnEnemy = (difficulty, forcedType = null, t = 0) => {
  ...
  if (forcedType === 'pylon') {
    const hp = Math.round((3510 + difficulty * 598) * ELITE_HP_MULT * diffHp);
    return {
      ...base,
      type: 'pylon',
      hp,
...
const beat = plan.beats[plan.idx];
    ...
    if (beat.kind === 'EVENT') {
      ...
      if (id === EVENT_ELITE_WALL) {
          ...
          const lateWallHp = t >= 0.50 ? 1.30 : 1;
          enemiesRef.current = [...(enemiesRef.current || []), ...spawnWallSweep(pp, difficulty, { hpMult: 3.225 * lateWallHp, speed: t >= 0.50 ? 1.04 : 0.92, count: 58, size: 124 }, t)];
      ...
      if (id === 'WALL') {
        const meta = {
          ...
          minRadius: 135,
          hpMult: (3.825 + Math.random() * 2.325) * (t >= 0.50 ? 1.30 : 1),
          size: 132,
          spawned: true
        };
    ...
    const loop = (frameTs = 0) => {
      ...
      const movedEnemiesRaw = nextEnemies.map((en) => {
        ...
        if (en.type === 'pylon') {
          ...
          const timeCharge = clamp(age / (en.pylonChargeMs || 9000), 0, 1);
